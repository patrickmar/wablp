...
export function WebinarsPage({
    ...
    const fetchPlatforms = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/routes/webinars/platforms`
        );
  ...
  const fetchWebinars = async (filters: any = {}) => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/routes/webinars`,
        { params: filters }
      );
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Select value={platformFilter} onValueChange={setPlatformFilter}>
              ...
              <SelectContent>
                ...
                {platforms.map((p) => (
                  <SelectItem
                    key={p.webinar_platforms_id}
                    value={String(p.webinar_platforms_id)}
                  >
                    {p.name}
                  </SelectItem>
                ))}
      ...
      <div className="flex items-center justify-between text-sm text-gray-600">
        <p>
          Showing {webinars.length} webinar{webinars.length !== 1 ? "s" : ""}{" "}
          found
        </p>
        {totalPages > 1 && (
          ...
          <Card key={w.webinars_id} className="hover:shadow-lg transition">
            ...
            <CardContent className="p-4">
              ...
              <h3
                ...
                onClick={() => onSelectWebinar(w.webinars_id)}
              >
                {w.name}
              </h3>
