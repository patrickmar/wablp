...
export function JobsPage({ onSelectJob }: JobsPageProps) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/routes/jobs/categories`);
  ...
  const fetchJobs = async (filters: any = {}) => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/routes/jobs`, { params: filters });
      setJobs(res.data);
    } catch (err) {
